...
const githubUrl = "https://github.com/Taboudi/Cat-Royale";
  ...
  return (
    <header className="site-header">
      <nav className="desktop-nav" aria-label="Main navigation">
        ...
        <a href={githubUrl} target="_blank" rel="noopener noreferrer" className="nav-link">GitHub Page</a>
      </nav>
      ...
      <details ref={menuRef} className="mobile-menu" onKeyDown={(event) => { if (event.key === "Escape") { closeMenu(); menuRef.current?.querySelector("summary")?.focus(); } }}>
        ...
        <nav className="mobile-nav" aria-label="Mobile navigation">
          ...
          <a href={githubUrl} target="_blank" rel="noopener noreferrer" className="nav-link" onClick={closeMenu}>GitHub Page</a>
        </nav>
      </details>

      <Link href="/" className="site-wordmark" aria-label="Cat Royale homepage">Cat Royale</Link>

      <button type="button" className="icon-button sound-toggle" onClick={toggleSound} aria-label={soundEnabled ? "Mute sound" : "Enable sound"} title={soundEnabled ? "Sound on" : "Sound off"}>
